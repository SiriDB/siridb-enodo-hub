...
class SerieManager:
    ...
    async def add_serie(cls, serie):
        if serie.get('name') not in cls._series:
            collected_datapoints = await query_serie_datapoint_count(ServerState.siridb_data_client, serie.get('name'))
    ...
    async def get_series_to_dict(cls, regex_filter=None):
        if regex_filter is not None:
            ...
            return [await serie.to_dict() for serie in cls._series.values() if pattern.match(serie.name)]
    ...
    async def add_anomalies_to_serie(cls, serie_name, points):
        ...
        if serie is not None:
            ...
            await drop_serie(ServerState.siridb_forecast_client, f'anomalies_{serie_name}')
            await insert_points(ServerState.siridb_forecast_client, f'anomalies_{serie_name}', points)
            await serie.set_detect_anomalies_status(DETECT_ANOMALIES_STATUS_DONE)

    @classmethod
    async def get_serie_forecast(cls, serie_name):
        values = await query_serie_data(ServerState.siridb_forecast_client, f'forecast_{serie_name}')
    ...
    async def get_serie_anomalies(cls, serie_name):
        values = await query_serie_data(ServerState.siridb_forecast_client, f'anomalies_{serie_name}')
        if values is not None:
